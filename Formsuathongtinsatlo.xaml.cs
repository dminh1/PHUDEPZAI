...
{
    public partial class Formsuathongtinsatlo : Window
    {
        ...
        public Formsuathongtinsatlo(string ID)
        {
            InitializeComponent();
            selectedID = ID;
        }

        private void cbo_select_Tinh(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
        ...
        private void select_quanhuyen(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
        ...
        private void Formsuathongtinsatlo_loaded(object sender, RoutedEventArgs e)
        {
            ...
            // Tự định nghĩa hàm LoadDataForEditing để hiển thị dữ liệu trên form (đã gọi ở constructor)
        }
        ...
        private void btn_ok_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                string update_query_satlo = "UPDATE quanlythongtinsatlo SET DiaChi = @DiaChi, ThoiGian = @ThoiGian, MucDoThienTai = @MucDoThienTai WHERE IDbaocao = @IDbaocao";
                ...
                using (connect_sql = new SqlConnection(sqlConnectString))
                {
                    ...
                    SqlCommand sqlcomand_datasatlo = new SqlCommand(update_query_satlo, connect_sql);
                    sqlcomand_datasatlo.Parameters.AddWithValue("@DiaChi", $"{cbo_xomkhoi.Text},{cbo_xaphuong.Text},{cbo_quanhuyen.Text},{cbo_Tinh.Text}");
                    sqlcomand_datasatlo.Parameters.AddWithValue("@ThoiGian", dtp_real.Text);
                    sqlcomand_datasatlo.Parameters.AddWithValue("@MucDoThienTai", cbo_mucdothientai.Text);
                    sqlcomand_datasatlo.Parameters.AddWithValue("@IDbaocao", selectedID);

                    string ten_bao_cao = string.Format(cbo_Tinh.Text.ToString() + "-" + dtp_real.Text.ToString() + "- Sạt lở " + cbo_mucdothientai.Text.ToString(), tb_linkbaibao.Text.ToString());
                    ...
                    if (sqlcomand_datasatlo.ExecuteNonQuery() == 1 && sqlcomand_databaocao.ExecuteNonQuery() == 1)
                    {
